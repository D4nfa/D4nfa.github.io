class Boid{
	constructor(pos = new vector2()){
		this.pos = pos;
		this.vel = new vector2(Math.ceil(Math.random() * 10) * (Math.round(Math.random()) ? 1 : -1), Math.ceil(Math.random() * 10) * (Math.round(Math.random()) ? 1 : -1));
		this.maxAccel = 0.012;
	...
	applyLogic(){
		...
		let coh = this.cohesion(20).mult(atractForce.value);;
		this.accel = this.accel.addVect(coh);

		let align = this.align(20).mult(alignForce.value);;
		this.accel = this.accel.addVect(align);

		this.accel = this.accel.setMag(0.5);
	}
	...
	align(range){
		...
		if(boids.length < 1){return new vector2();}
